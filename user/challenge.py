...
#-------------------------------------------------------------------------------------------
random_init = list()
if len(sys.argv) > 2 and sys.argv[2][:6] == "-right":
  random_init.extend([0, flatten_width/2-1])
elif len(sys.argv) > 2 and sys.argv[2][:5] == "-left":
  ...
  if i in init:
    ...
    if len(random_init) == 2:
      for k in range(len(inputs)):
        ...
        elif len(sys.argv) > 2 and sys.argv[2] in ["-right-noisy","-left-noisy"] and inputs[k] == 0:
          if len(sys.argv) == 3:
            inputs[k] = random.randint(0,250)
          elif len(sys.argv) > 4:
            if 1 <= random.randint(0,int(sys.argv[4])) <= int(sys.argv[3]):
              inputs[k] = random.randint(0,250)
    #------------------
